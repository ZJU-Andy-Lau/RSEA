...
class PretrainDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for dataset_idx,key in enumerate(self.database_keys):
            ...
            imgs1 = torch.from_numpy(np.stack([image_1_full[tl[0]:tl[0] + self.input_size,tl[1]:tl[1] + self.input_size] for tl in windows],axis=0)).permute(0,3,1,2)
            imgs2 = torch.from_numpy(np.stack([image_2_full[tl[0]:tl[0] + self.input_size,tl[1]:tl[1] + self.input_size] for tl in windows],axis=0)).permute(0,3,1,2)
