...
class CriterionFinetune(nn.Module):
    ...
    def forward(self,epoch,max_epoch,
        ...
        dis_positive = torch.concatenate([torch.norm(feat1_PD - feat2_PD,dim=1),
                                           torch.norm(feat2_PD - feat1_PD,dim=1)])
        dis_negative = torch.concatenate([torch.norm(feat1_PD - feat1_negative,dim=1),
        ...
        # loss_feat = torch.clip(simi_negative - simi_positive + .5,min=0.).mean() * 10000 + (1. - simi_positive.mean()) * 20000
        loss_feat = torch.clip(dis_positive - dis_negative + 2.,min=0.).mean() * 1000 + dis_positive.mean() * 2000
        ...
        return loss,loss_obj,loss_height,loss_relative,loss_conf,loss_feat,residual_threshold,dis_positive.mean(),dis_negative.mean()
