...
class RPCModelParameterTorch:
    ...
    # def rpc_monomials(self,Xn, Yn, Zn):
    #     """
    #     Compute 20 normalized monomials for RPC using PyTorch tensors.
    #     Order: 1, Xn, Yn, Zn, Xn*Yn, Xn*Zn, Yn*Zn, Xn^2, Yn^2, Zn^2,
    #         Yn*Xn^2, Xn*Yn^2, Zn*Xn^2, Xn*Zn^2, Zn*Yn^2, Yn*Zn^2,
    #         Xn*Yn*Zn, Xn^3, Yn^3, Zn^3
    #     """
    #     device = self.LNUM.device
    #     dtype = self.LNUM.dtype
    #     ones = torch.ones_like(Xn).to(dtype=dtype,device=device)
    #     terms = [
    #         ones,
    #         Xn, Yn, Zn,
    #         Xn * Yn, Xn * Zn, Yn * Zn,
    ...
    #         Xn**3, Yn**3, Zn**3
    #     ]
    #     return torch.stack(terms, dim=1).to(dtype=dtype,device=device)  # shape (N,20)

    # # Main function: modify rpc object in place
    # def rectify_rpc(self,affine, num_points=2000, max_iter=5, sigma_thresh=3.0):
    #     """
    #     rpc: instance with attributes:
    #         .LINE_OFF, .SAMP_OFF, .LAT_OFF, .LONG_OFF, .HEIGHT_OFF (scalars)
    #         .LINE_SCALE, .SAMP_SCALE, .LAT_SCALE, .LONG_SCALE, .HEIGHT_SCALE (scalars)
    #         .LNUM, .LDEM, .SNUM, .SDEM (arrays length 20)
    #     affine: tuple (a0, a1, a2, a3, a4, a5)
    #     num_points: number of sample points in [-1,1]^3
    #     max_iter: max iterations for outlier rejection
    #     sigma_thresh: threshold multiplier for residual
    #     This function updates rpc.LNUM, rpc.LDEM, rpc.SNUM, rpc.SDEM in place.
    #     """
    #     device = self.LNUM.device
    #     dtype = self.LNUM.dtype
    #     # Unpack image normalization
    #     Loff = self.LINE_OFF
    #     Soff = self.SAMP_OFF
    #     Lscale = self.LINE_SCALE
    #     Sscale = self.SAMP_SCALE
    ...
    #     # Sample normalized ground coords uniform[-1,1]
    #     torch.manual_seed(0)
    #     Xn = (torch.rand(num_points, device=device, dtype=dtype) * 2 - 1)
    #     Yn = (torch.rand(num_points, device=device, dtype=dtype) * 2 - 1)
    #     Zn = (torch.rand(num_points, device=device, dtype=dtype) * 2 - 1)

    #     # Compute monomials
    #     M = self.rpc_monomials(Xn, Yn, Zn)  # (N,20)

    #     # Original normalized image coords
    #     Ln0 = (M @ p0) / (M @ q0)
    #     Sn0 = (M @ r0) / (M @ s0)

    #     # Denormalize to pixel coords
    #     l0 = Ln0 * Lscale + Loff
    #     s_pix = Sn0 * Sscale + Soff

    #     # Affine
    #     a0, a1, a2, a3, a4, a5 = [torch.tensor(a, device=device, dtype=dtype) for a in affine]
    #     lp = a0 * l0 + a1 * s_pix + a2
    #     sp = a3 * l0 + a4 * s_pix + a5
    ...
    #         A_lin = torch.cat([M_sub, -Ln_sub.unsqueeze(1) * M_sub], dim=1)  # (n,40)
    #         A_sam = torch.cat([M_sub, -Sn_sub.unsqueeze(1) * M_sub], dim=1)

    #         # Solve via pseudo-inverse
    #         # coef = pinv(A) @ target
    #         coef_lin = torch.linalg.pinv(A_lin) @ Ln_sub
    #         coef_sam = torch.linalg.pinv(A_sam) @ Sn_sub

    #         p_new, q_new = coef_lin[:20], coef_lin[20:]
    #         r_new, s_new = coef_sam[:20], coef_sam[20:]

    #         # Compute fitted coords & residuals
    #         Ln_fit = (M @ p_new) / (M @ q_new)
    #         Sn_fit = (M @ r_new) / (M @ s_new)
    #         res = torch.sqrt((Ln_fit - Ln_p)**2 + (Sn_fit - Sn_p)**2)
    #         sigma = torch.std(res[mask].float())
    #         new_mask = res < sigma_thresh * sigma
    #         if torch.all(new_mask == mask):
    #             break
    #         mask = new_mask
