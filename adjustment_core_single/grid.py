...
def visualize_grid_selection(args, all_candidate_info: List[Dict], selected_diags: List[np.ndarray], ref_image: RSImage, level: int):
    ...
    print(f"Generating grid selection visualization for Level {level}...")
    try:
        # 1. 获取参考影像的地理边界
        ...
        if geo_w == 0 or geo_h == 0:
            print("Invalid geographic bounds for visualization.")
            return
        ...
        print(f"Saved grid selection visualization to {output_path}")

    except Exception as e:
        print(f"FAILED to generate grid visualization. Error: {e}")
...
def select_grids_by_confidence(args, 
    ...
    if not args.auto:
        # --- [修改] 日志：Rank {local_rank} -> SingleGPU ---
        print(f"[SingleGPU] Strategy = Confidence-based selection...")
        print(f"[SingleGPU] Using pre-loaded encoder for grid quality assessment...")
    ...
    if not args.auto:
        # --- [修改] 日志：Rank {local_rank} -> SingleGPU ---
        print(f"[SingleGPU] Assessing quality for {len(all_candidate_diags)} candidate grids...")
    resample_size = 1024
        ...
        for diag in grid_iter:
            ...
            for img in images:
                try:
                    ...
                    img_tensor = transform_assess(img_patch)[None].to(device)
    ...
    if num_to_select > 0 and len(all_valid_grids_info) > num_to_select:
        if not args.auto:
            # --- [修改] 日志：Rank {local_rank} -> SingleGPU ---
            print(f"[SingleGPU] Found {len(all_valid_grids_info)} valid grids. Selecting {num_to_select} using spatial selection...")
        ...
        if not args.auto:
             # --- [修改] 日志：Rank {local_rank} -> SingleGPU ---
            print(f"[SingleGPU] Using suppression radius {suppression_radius:.2f} m...")
        ...
        if num_selected_by_nms < num_to_select:
            if not args.auto:
                # --- [修改] 日志：Rank 0 -> SingleGPU ---
                print(f"[SingleGPU] NMS 选中了 {num_selected_by_nms} 个格网 (目标: {num_to_select})。")
                print(f"[SingleGPU] 正在从高置信度列表中补齐剩余格网...")
            ...
            if not args.auto:
                print(f"[SingleGPU] 已补齐 {len(backfill_grids_info)} 个格网。")
        ...
        if not args.auto:
             # --- [修改] 日志：Rank {local_rank} -> SingleGPU ---
             print(f"[SingleGPU] num_to_select ({num_to_select}) 为 0 或 >= 有效格网总数。使用所有 {len(all_valid_grids_info)} 个有效格网。")
...
def select_grids_uniformly(args, 
    ...
    if not args.auto:
        # --- [修改] 日志：Rank {local_rank} -> SingleGPU ---
        print(f"[SingleGPU] Strategy = Uniform selection (fast, reproducible).")
    ...
    if num_to_select > 0 and num_candidates > num_to_select:
        if not args.auto:
            # --- [修改] 日志：Rank {local_rank} -> SingleGPU ---
            print(f"[SingleGPU] Found {num_candidates} grids. Sorting and selecting {num_to_select} uniformly...")
        ...
        if not args.auto:
            # --- [修改] 日志：Rank {local_rank} -> SingleGPU ---
            print(f"[SingleGPU] Using all {num_candidates} grids (num_to_select ({num_to_select}) is 0 or >= num_candidates).")
