...
def load_imgs_bundle(args) -> List[RSImage]:
    ...
    if not args.auto:
        print(f"[SingleGPU] Found {len(img_folders)} image folders. Loading all...")
    ...
    for idx, folder in enumerate(img_folders):
        ...
        try:
            ...
            # --- [修改] 移除 dist.get_rank() == 0 判断 ---
            if not args.auto:
                print(f"[SingleGPU] Loaded image {idx} from {folder}.")
        except Exception as e:
            # --- [修改] 移除 dist.get_rank() 判断 ---
            print(f"[SingleGPU] Failed to load image {idx} from {folder}: {e}")
    

    # --- [修改] 移除 dist.get_rank() == 0 判断 ---
    if not args.auto:
        print(f"[SingleGPU] Successfully loaded {len(images)} images into memory.")
    return images
...
class TqdmLogger:
    ...
    def __init__(self, args, total_iters: int, level: int, local_rank: int):
        ...
        # --- [修改] is_rank_0 始终为 True ---
        self.is_rank_0 = True #(local_rank == 0)
